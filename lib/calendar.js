/**
 * Copyright 2018 Fin2B Inc. (http://fin2b.com/)
 * All Right Reserved.
 *
...
module.exports = {
   ...
   * @return result {Array} The array of date object.
   */
  getBusinessDays: function (params, cb) {
    if (params.year) {
      if (typeof params.year === "string" && params.year.length === 4) {
        if (Number(params.year) > 2022 || Number(params.year) < 2014) {
          return cb("params error: year from '2014' to '2022' required");
        }
    ...
    var day = '';
    var type = params.type ? params.type : 'h,i';
    ...
    request(options, function (err, resp, body) {
      ...
      if (!err && resp.statusCode == 200) {
        ...
        _.forEach(specialDays, function (value) {
          ...
          if (type === 'h,i') {
            getDaysAndWeekends[index].type = value.type;
          } else {
        ...
        if(resp.statusCode == 401) {
          var errMsg = JSON.parse(body).error.message;
